...
export default function PaymentContractModal({
  ...
  const summary = useMemo(() => {
    if (!activeContract) return { total: 0, taxaTotal: 0, principalTotal: 0, count: 0, descricao: "" };

    if (!isParcelado) {
      const juros = Number(activeContract.valorPrincipal) * (Number(activeContract.jurosPercent) / 100);
      const taxaContrato = Number(activeContract.taxa || 0);
      const taxaEfetiva = isCustomPayment ? Number(customTaxValue || 0) : taxaContrato;

      return {
        total: juros + taxaEfetiva,
  ...
  const handleQuit = () => {
    ...
    let valorQuitacao = Number(activeContract.valorEmAberto) + Number(activeContract.taxa);
    if (!isParcelado) {
      const juros = Number(activeContract.valorPrincipal) * (Number(activeContract.jurosPercent) / 100);
      valorQuitacao += juros;
    }
    paymentMutation.mutate({ tipo: "MISTO", valorPago: valorQuitacao, observacao: "Quitação Total" });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => (!v ? onClose() : null)}>
      {/* [&>button]:hidden remove o botão de fechar nativo do Radix */}
      <DialogContent className="max-w-5xl h-[100dvh] sm:h-[90vh] bg-[#071e30] border-white/10 text-white p-0 overflow-hidden flex flex-col [&>button]:hidden">

        {/* CABEÇALHO EM LINHA (FLEX-ROW) CONFORME EXEMPLO */}
        ...
        {isLoading ? (
          ...
          <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
            ...
            {/* LADO ESQUERDO: PARCELAS */}
            <div className="flex-1 flex flex-col overflow-hidden bg-black/20">
              {!isParcelado ? (
                <div className="flex flex-col items-center justify-center h-full text-gray-400 p-10 text-center">
                  <Calendar className="w-16 h-16 mb-4 opacity-20" />
                  <h3 className="text-lg font-medium text-white mb-2">Contrato Mensal</h3>
                  <p className="max-w-xs">Renovação de juros e multas de atraso.</p>
                </div>
              ) : (
                <>
            ...
            {/* LADO DIREITO: RESUMO */}
            <div className="shrink-0 w-full md:w-[350px] bg-[#020617] border-t md:border-t-0 md:border-l border-white/10 flex flex-col p-4 z-20 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] md:shadow-none">
              <div className="mb-4">
                <div className="space-y-3 bg-white/5 p-3 rounded-lg border border-white/10">
                  <div className="flex justify-between items-center text-xs sm:text-sm">
                    <span className="text-gray-400">Principal</span>
                    <span className="text-white">{formatCurrency(summary.principalTotal)}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs sm:text-sm">
                    <span className="text-gray-400">Multas/Taxas</span>
                    <div className="flex items-center gap-2">
                      <span className={cn(summary.taxaTotal > 0 ? "text-red-400" : "text-gray-400")}>{formatCurrency(summary.taxaTotal)}</span>
                      <AlertDialog>
                  ...
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-xl sm:text-2xl font-bold text-gold">{formatCurrency(summary.total)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 mt-auto">
                ...
                >
                  ...
                  {isCustomPayment ? "Cancelar Ajuste" : "Personalizar Valor"}
                </Button>
                ...
                >
                  {paymentMutation.isPending ? <Loader2 className="animate-spin" /> : (
                    <div className="flex items-center justify-between w-full px-2">
                      <span className="text-sm">{isCustomPayment ? "Confirmar" : "Pagar Seleção"}</span>
